...
def generate_subgraph_datasets(params, splits=['train', 'valid'], saved_relation2id=None, max_label_value=None):
    ...
    saved_relation2id = True
    if saved_relation2id:
        print('111')
        entity_save_path = os.path.join(params.main_dir, 'data/{}/id2entity.dict'.format(params.dataset))
        rel_save_path = os.path.join(params.main_dir, 'data/{}/relation2id.dict'.format(params.dataset))
        with open(entity_save_path, "w") as file:  
            json.dump(id2entity, file)
        with open(rel_save_path, "w") as file:  
            json.dump(relation2id, file)
    ...
    for split_name in splits:
        ...
        protein_in_head = np.isin(triplets[split_name][:, 0], protein_list)
        GO_in_tail = np.isin(triplets[split_name][:, 1], go_list)

        mask = np.logical_and(protein_in_head, GO_in_tail)
        graphs[split_name]['P_G_relation'] = mask
        filtered_triplets = graphs[split_name]['triplets'][mask]
        graphs[split_name]['triplets'] = filtered_triplets

    for split_name, split in graphs.items():
        logging.info(f"Sampling negative links for {split_name}")
        split['pos'], split['neg'] = sample_neg(go_list, protein_list, adj_list, split['triplets'], params.num_neg_samples_per_link, max_size=split['max_size'], constrained_neg_prob=params.constrained_neg_prob)
    if testing:
...
class SubgraphDataset(Dataset):
    ...
    def draw_graph(self, data_pos):  
        # get head and tail  
        indices_head = torch.nonzero(data_pos.ndata['id'] == 1)  
        indices_head_list = list(indices_head.numpy())  
        indices_tail = torch.nonzero(data_pos.ndata['id'] == 2)  
        indices_tail_list = list(indices_tail.numpy())  
        
        nx_g = dgl.to_networkx(data_pos, node_attrs=['id'])
