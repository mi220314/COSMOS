...
def sample_neg(go_list, protein_list, adj_list, edges, num_neg_samples_per_link=1, max_size=1000000, constrained_neg_prob=0):
    pos_edges = edges
    neg_edges = []
    ...
    while len(neg_edges) < num_neg_samples_per_link * len(pos_edges):
        ...
        valid_protein = pos_edges[:,0]
        valid_go = pos_edges[:,1]
        if np.random.uniform() < constrained_neg_prob:
            if np.random.uniform() < 0.5:
                neg_head = np.random.choice(valid_protein)
            else:
                neg_tail = np.random.choice(valid_go)
        else:
            if np.random.uniform() < 0.5:
                neg_head = np.random.choice(valid_protein)
            else:
...
def links2subgraphs(A, graphs, params, max_label_value=None):
    ...
    add graphs[split_name]["P_G_relation"]: np.array(bool) to denote whether it's a P-G relation
    '''
    ...
    for split_name, split in graphs.items():
        links_length += (len(split['pos']) + len(split['neg'])) * 4
    map_size = math.ceil(links_length * BYTES_PER_DATUM)
    ...
    def extraction_helper(A, links, g_labels, split_env):
        ...
        with mp.Pool(processes=4, initializer=intialize_worker, initargs=(A, params, max_label_value)) as p:
...
def subgraph_extraction_labeling(ind, rel, A_list, h=1, enclosing_sub_graph=False, max_nodes_per_hop=None, max_node_label_value=None):
    """
    The function get the n_labels=pruned_labels, takes the distance<=3 as the upper limit to get the subgraph
    """
