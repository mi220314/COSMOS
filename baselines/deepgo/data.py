...
def load_data(
        data_root, ont, terms_file, dataset, features_length=2560,
    ...
    train_df = pd.read_pickle(f'{data_root}/{ont}/{dataset}/train_data_{dataset}.pkl')
    valid_df = pd.read_pickle(f'{data_root}/{ont}/{dataset}/valid_data_{dataset}.pkl')
    test_df = pd.read_pickle(f'{data_root}/{ont}/{dataset}/{test_data_file}')
    # print(test_df.columns) # Index(['Protein', 'interpros', 'prop_annotations', 'sequences', 'esm2']
    # print(test_df['prop_annotations'].head())
    # print(test_df['interpros'].head())
...
def load_ppi_data(data_root, ont, dataset, features_length=2560,
                  features_column='esm2', test_data_file='test_data.pkl',
    ...
    train_df = pd.read_pickle(f'{data_root}/{ont}/{dataset}/train_data_{dataset}.pkl')
    valid_df = pd.read_pickle(f'{data_root}/{ont}/{dataset}/valid_data_{dataset}.pkl')
    test_df = pd.read_pickle(f'{data_root}/{ont}/{dataset}/test_data_{dataset}.pkl')

    df = pd.concat([train_df, valid_df, test_df])  # len = 21888
    ...
    graph = graphs[0] # nodes = 14820, train valid proteins may be duplicated, but the test are disjoint
...
def load_normal_forms(go_file, terms_dict):
    ...
    # print(zclasses['GO:0044502']) #is GO-id to id, is 40000+ for regular while 70000+ for plus
