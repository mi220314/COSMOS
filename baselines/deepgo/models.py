...
class DeepGOModel(BaseDeepGOModel):
    ...
    def predict_zero(self, features, data):
        """
        zero shot prediction function from deepgozero
        """
        x = self.net(features)
...
class DeepGOGATModel(BaseDeepGOModel):
    ...
    def predict_zero(self, input_nodes, output_nodes, blocks, godata):
        """
        Forward pass of the DeepGOGAT Model.
        ...
        x = self.net1(features)
        x = self.conv1(g1, x).squeeze(dim=1)
        go_embed = self.go_embed(godata)
        hasFunc = self.rel_embed(self.hasFuncIndex)
        hasFuncGO = go_embed + hasFunc
        go_rad = th.abs(self.go_rad(godata).view(1, -1))
        x = th.matmul(x, hasFuncGO.T) + go_rad
        logits = th.sigmoid(x)
        return logits
...
class MLPModel(nn.Module):
    ...
    def predict_zero(self, features, go_data):
        return self.net(features)
