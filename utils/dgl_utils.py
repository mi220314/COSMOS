...
def _bfs_relational(adj, roots, max_nodes_per_hop=None):
    ...
    while current_lvl:
        ...
        if max_nodes_per_hop and max_nodes_per_hop * len(roots) < len(next_lvl):
            # print("reach the limit, sample max_nodes_per_hop")
            next_lvl = set(random.sample(list(next_lvl), max_nodes_per_hop * len(roots)))
