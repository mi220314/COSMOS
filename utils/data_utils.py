...
def plot_rel_dist(adj_list, filename):
    rel_count = []
    """
    for each adj, count the total nonzero value in the sparse matrix, 
    the length of rel_count should be len(adj_list), namely, the number of relations
    """
    for adj in adj_list:
...
def process_files(files, saved_relation2id=None):
    ...
    with open("../data/id2entity.dict", "w") as file:  
        json.dump(entity2id, file)
    with open("../data/id2relation.dict", "w") as file:  
